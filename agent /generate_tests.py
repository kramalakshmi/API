...
Just clean, minimal test code.
    ...
    response = client.chat.completions.create(
        model=MODEL,
        messages=[
            {"role": "system", "content": "You are an expert Python developer. Rrturn  only the code,do not include notes, do not include explanations ,do not include   comments, do not include docstrings.
...
def commit_file(path, content):
    ...
    try:
        ...
        repo.update_file(
            path, "Update generated tests", content, branch= "Agents"
        )
    except:
        repo.create_file(
