...
def commit_file(path, content):
    ...
    try:
        ...
        '''
        existing = repo.get_contents(path,ref="MoreRefined")
        print("Path "+ path)
        
        repo.update_file(
            path, "Update generated tests", content, existing.sha, branch= "MoreRefined"
        )
    except:
        repo.create_file(
            path, "Add generated tests", content , branch= "MoreRefined"
        )
